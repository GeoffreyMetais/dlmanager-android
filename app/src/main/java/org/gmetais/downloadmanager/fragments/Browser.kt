...
class Browser : BaseBrowser(), BrowserAdapter.IHandler {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mCurrentDirectory.dataResult.observe(this, Observer<ApiRepo.Result> { update(it!!) })
        showProgress()
    }
    ...
    private fun update(result: ApiRepo.Result) {
        ...
        when (result) {
            is ApiRepo.Result.Success<*> -> (mBinding.filesList.adapter as BrowserAdapter).update((result.content as Directory).files.sortedBy { !it.isDirectory })
